...
class DialogDeleteDownloaded(private val context: Context) {
    private val binding by lazy {
        DialogDeleteDownloadedBinding.inflate(LayoutInflater.from(context))
    }
    ...
    fun show(onYes : () -> Unit) {
        binding.btnYes.clickSafe {
            onYes()
            dialog.dismiss()
        }
        binding.btnNo.clickSafe {
            hide()
        }
        if (!dialog.isShowing)
            dialog.show()
